      ...
      implements EasyScoreCalculator<TowerSchedule>
{
   ...
   // pri.add(i);
   // for (GridLocation loc : checkStrictPriority(pri,
   // locations.getTowerGrid().getGrid())) {
   // best.add(loc);
   // }
   // }
      ...
      for (Iterator<CellPhone> itr = priorityPhones.iterator(); itr.hasNext();)
      {
         ...
         for (CellTower tower : locations.getTowerList())
         {
            if (tower.getPhonesServiced() != null
                  && tower.getPhonesServiced().contains(phone))
            {
   ...
   */
   ...
   // private List<GridLocation> checkStrictPriority(List<Integer> pri,
