	...
	public TowerGrid(double range, double topLeftCornerLat, double topLeftCornerLng, double botLeftCornerLat,
		...
		while (currentLat > botLeftCornerLat) {
			...
			for (CellPhone phone : phones) {
				if (metersBetweenTwoPoints(currentLat, currentLng, phone.getLat(), phone.getLng()) <= 9000.00
						+ phone.getRange()) {
...
//		}
	...
	public void makeInitialGuess(int numTowers, List<GridLocation> grid) {
		...
		while (count < numTowers) {
			...
			System.out.println("Best score is now "+bestScore+" plus "+gridInProgress.get(0).getScore()+" from grid ID "+gridInProgress.get(0).getId());
	...
	private void setLocationScore(GridLocation loc) {
		...
		for (CellPhone phone : this.phonesInGrid) {
			if (metersBetweenTwoPoints(loc.getLat(), loc.getLng(), phone.getLat(), phone.getLng()) <= 9000.00
					+ phone.getRange()) {
