...
public class CellTowerApp
{
   ...
   public static void main(String[] args)
   {
      ...
      SolverFactory<TowerSchedule> solverFactory = SolverFactory
            .createFromXmlResource(
                  "org/optaplanner/examples/celltower/solver/CellTowerSolverConfig.xml");
      ...
      {
         solveForPhonesInArea(solverFactory, 10, 2, 4000.00, 37.00, -106.00,
               36.00, -105.00);
      }

   }

   private static void solveSmallArea(
         SolverFactory<TowerSchedule> solverFactory)
   {
      Solver<TowerSchedule> solver = solverFactory.buildSolver();
      TowerSchedule unsolvedTowerSchedule = new TowerScheduleGenerator()
            .createSmallTowerSchedule();
   ...
   private static void solveLargeArea(
         SolverFactory<TowerSchedule> solverFactory)
   {
      Solver<TowerSchedule> solver = solverFactory.buildSolver();
      TowerSchedule unsolvedTowerSchedule = new TowerScheduleGenerator()
            .createMediumTowerSchedule();
      TowerSchedule solvedTowerSchedule = null;
      solver.solve(unsolvedTowerSchedule);
      solvedTowerSchedule = (TowerSchedule) solver.getBestSolution();
      printTowerScheduleinGeoJSON(solvedTowerSchedule);
   ...
   private static void solveForPhonesInArea(SolverFactory<TowerSchedule> solverFactory,
         int phones, int towers, double phoneRange, double topLeftLat,
         double topLeftLng, double bottomRightLat, double bottomRightLng)
   {

      Solver<TowerSchedule> solver = solverFactory.buildSolver();
      TowerSchedule unsolvedTowerSchedule = new TowerScheduleGenerator()
            .createTowerSchedule(phones, towers, phoneRange, topLeftLat,
                  topLeftLng, bottomRightLat, bottomRightLng);
         ...
         TowerSchedule solvedTowerSchedule)
   {
      ...
      try
      {
         ...
         List<List<GridLocation>> latLines = solvedTowerSchedule.getTowerGrid()
               .getLatLines();
         ...
         for (CellTower placedTower : initialGuess)
         {
            ...
            for (int i = 0; i < 8; i++)
            {
               polyPoints[0][i] = placedTower.getCirclePoints()
      ...
      {
         System.out.println("This tower is at " + placed.getLat() + " lat and "
               + placed.getLng() + " lng. This tower has a total score of "
               + placed.getScore());
